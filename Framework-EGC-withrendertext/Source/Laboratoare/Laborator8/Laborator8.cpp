...
void Laborator8::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
	...
	int type = glGetUniformLocation(shader->program, "type");
	glUniform1i(type, typeOfLight);

	int cut_off_angle = glGetUniformLocation(shader->program, "angle");
	glUniform1f(cut_off_angle, angle);
...
void Laborator8::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
	{
		...
		if (window->KeyHold(GLFW_KEY_I)) angleOX += deltaTime * speed * 2;
		if (window->KeyHold(GLFW_KEY_K)) angleOX -= deltaTime * speed * 2;
		if (window->KeyHold(GLFW_KEY_J)) angleOY += deltaTime * speed * 2;
		if (window->KeyHold(GLFW_KEY_L)) angleOY -= deltaTime * speed * 2;


		if (window->KeyHold(GLFW_KEY_N)) angle += deltaTime * speed * 5;
		if (window->KeyHold(GLFW_KEY_M)) angle -= deltaTime * speed * 5;
