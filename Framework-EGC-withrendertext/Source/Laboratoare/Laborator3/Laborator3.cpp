...
void Laborator3::Init()
{
	...
	Mesh* square3 = Object2D::CreateSquare("square3", corner, squareSide, glm::vec3(1, 1, 0), true);
...
/*
	...
	// translate
	translateX += 30 * deltaTimeSeconds;
	...
	float x = cx - squareSide / 2;
	float y = cy - squareSide / 2;

	angularStep = angularStep + deltaTimeSeconds * 0.5;
...
*/
	...
	// earth
	angularStep = angularStep + deltaTimeSeconds * 0.2;

	float earthX = cx + 200; //550-350

	//revolution of the earth around the sun
	modelMatrix *= Transform2D::Translate(earthX, cy);
	modelMatrix *= Transform2D::Rotate(angularStep);
	modelMatrix *= Transform2D::Translate(-earthX, -cy);
	...
	float moonX = cx + 150; //350-200
	earthX = cx + 200;

	//revolution of the moon around the earth
	modelMatrix *= Transform2D::Translate(moonX, cy);
	//modelMatrix *= Transform2D::Translate(earthX, cy);
	modelMatrix *= Transform2D::Rotate(angularStep);
	modelMatrix *= Transform2D::Translate(-moonX, -cy);
	//modelMatrix *= Transform2D::Translate(-earthX, -cy);

	//rotation of moon around its own axis
	modelMatrix *= Transform2D::Translate(cx, cy);
	modelMatrix *= Transform2D::Rotate(angularStep);
	modelMatrix *= Transform2D::Translate(-cx, -cy);	
